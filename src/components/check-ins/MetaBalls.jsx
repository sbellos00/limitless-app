...
const MetaBalls = ({
  ...
  const attractAmountRef = useRef(0);
  attractRef.current = attractToCenter;
  ...
  useEffect(() => {
    ...
    function update(t) {
      ...
      // Smooth attract interpolation (read from ref, no re-render needed)
      const attractTarget = attractRef.current ? 1 : 0;
      attractAmountRef.current += (attractTarget - attractAmountRef.current) * 0.03;
      const attract = attractAmountRef.current;
      const effectiveClump = clumpFactor * (1 - attract);
      ...
      for (let i = 0; i < effectiveBallCount; i++) {
        ...
        const posX = x * p.baseScale * effectiveClump;
        const posY = y * p.baseScale * effectiveClump;
        const effectiveRadius = p.radius * (1 - attract * 0.4);
        metaBallsUniform[i].set(posX, posY, effectiveRadius);
      }

      // Cursor ball: center + grow when attracting, normal behavior otherwise
      let targetX, targetY;
      ...
      program.uniforms.iCursorBallSize.value = cursorBallSize + attract * 4;
